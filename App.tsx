    ...
    <nav className="fixed top-0 left-0 right-0 z-[100] w-full bg-slate-200/90 dark:bg-background-dark/80 backdrop-blur-md border-b border-slate-300 dark:border-white/5 h-16 transition-colors duration-300">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-full relative flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="h-6 w-px bg-slate-400/20 dark:bg-white/10 mx-1"></div>

          <button 
            onClick={scrollToContact}
            className="w-10 h-10 rounded-xl flex items-center justify-center border border-slate-400 dark:border-white/10 hover:border-primary/50 transition-all text-slate-700 dark:text-gray-400 hover:text-primary bg-slate-100 dark:bg-transparent shadow-inner dark:shadow-none group"
            title={t[lang].contactTitle}
          >
            <span className="material-symbols-outlined text-xl group-hover:-translate-y-0.5 transition-transform">mail</span>
          </button>

          <button 
            onClick={scrollToRegistry}
            className="w-10 h-10 rounded-xl flex items-center justify-center border border-slate-400 dark:border-white/10 hover:border-primary/50 transition-all text-slate-700 dark:text-gray-400 hover:text-primary bg-slate-100 dark:bg-transparent shadow-inner dark:shadow-none group"
            title={lang === 'ar' ? 'تقارير الثغرات العالمية (CVE)' : 'Global Vulnerability Reports (CVE)'}
          >
            <span className="material-symbols-outlined text-xl group-hover:scale-110 transition-transform">list_alt</span>
          </button>
        </div>

        <div className="absolute left-1/2 -translate-x-1/2 hidden md:flex items-center whitespace-nowrap gap-6">
          <button onClick={() => setCurrentPage('home')} className={`text-[13px] font-black uppercase tracking-widest transition-all ${currentPage === 'home' ? 'text-primary' : 'text-slate-600 dark:text-gray-500 hover:text-slate-900 dark:hover:text-white'}`}>{t[lang].home}</button>
          <button onClick={() => setCurrentPage('theory-list')} className={`text-[13px] font-black uppercase tracking-widest transition-all ${currentPage === 'theory-list' ? 'text-primary' : 'text-slate-600 dark:text-gray-500 hover:text-slate-900 dark:hover:text-white'}`}>{t[lang].theoryNav}</button>
          <button onClick={() => setCurrentPage('practical-list')} className={`text-[13px] font-black uppercase tracking-widest transition-all ${currentPage === 'practical-list' ? 'text-primary' : 'text-slate-600 dark:text-gray-500 hover:text-slate-900 dark:hover:text-white'}`}>{t[lang].practicalNav}</button>
          <button onClick={() => setCurrentPage('quiz-list')} className={`text-[13px] font-black uppercase tracking-widest transition-all ${currentPage === 'quiz-list' ? 'text-primary' : 'text-slate-600 dark:text-gray-500 hover:text-slate-900 dark:hover:text-white'}`}>{t[lang].quizNav}</button>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className="font-black text-2xl text-primary font-display select-none">{t[lang].settings}</span>
          </div>
        </div>
      </div>
    </nav>
          ...
          <div className="flex flex-col min-h-screen relative">
            ...
            >
              <div className="absolute inset-0 pointer-events-none z-10 p-10 opacity-30">
                 ...
                 <div className={`absolute top-20 ${lang === 'ar' ? 'left-10' : 'right-10'} w-20 h-20 border-l-2 border-t-2 border-slate-400/50 dark:border-primary/20`}></div>
                 <div className={`absolute bottom-10 ${lang === 'ar' ? 'right-10' : 'left-10'} w-20 h-20 border-r-2 border-b-2 border-slate-400/50 dark:border-primary/20`}></div>
              </div>
              ...
              <div className="relative z-20 max-w-[1400px] mx-auto px-4 w-full flex flex-col items-center">
                ...
                <div className="relative flex flex-col items-center text-center space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-1000">
                  ...
                  <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold leading-relaxed tracking-widest uppercase font-pixel text-slate-900 dark:text-white">
                    <span className="font-pixel text-primary drop-shadow-[0_4px_10px_rgba(0,0,0,0.2)] dark:drop-shadow-glow">
                      OWASP
                    </span>
                    <br/>
                    TOP 10
                  ...
                  <div className="flex justify-center pt-8">
                    <button 
                      ...
                      className="relative group overflow-hidden px-14 py-7 bg-slate-900 dark:bg-primary text-white dark:text-black font-black uppercase tracking-widest text-2xl rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-105"
                    >
            ...
            {/* GLOBAL INTELLIGENCE REGISTRY (MATCHING SCREENSHOT) */}
